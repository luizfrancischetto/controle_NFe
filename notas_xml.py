...
def _itens_nfe(inf):
    """Lista de itens <det> de uma NF-e, cada um com NCM e valor do produto."""
    itens = []
    for det in inf:
        ...
        if prod is None:
            continue
        itens.append({"ncm": _text(prod, "NCM"), "valor": parse_num(_text(prod, "vProd"))})
    return itens
...
def _parsear_nfe(inf, cnpj_empresa):
    ...
    # NCM: só preenche automaticamente quando a nota tem um único item, para
    # não atribuir um NCM errado a uma nota com produtos diferentes.
    itens = _itens_nfe(inf)
    ncm = itens[0]["ncm"] if len(itens) == 1 else ""
